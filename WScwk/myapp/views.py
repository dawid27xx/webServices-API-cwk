...
def listInstances(request):
    instances = list(ModuleInstance.objects.values())
...
def viewProfessors(request):
    ...
    for professor in professors:
        ...
        avgRating = round(avgRating, 2) if avgRating is not None else 0

        professorRatings[professor.professor_code] = {
...
def rateInstance(request):
    if request.method == "POST":
        try:
            ...
            if not all([professorId, moduleCode, year, semester, rating, userId]):
